...
function normalizeCalendarCountryCode(raw: string): string {
...
2) Support / resistance — one two-column table PER instrument/chart:
...
<table class="${ATFX_LEVELS_TABLE_CLASS}">…from technical analysis…</table>

Example support/resistance table:
...
- Do NOT use Level / Zone / Description columns. Use ONLY numeric prices from technical analysis in the table.`;
}
...
function buildLevelTable(rows: LevelRow[]): string {
  ...
  if (resistances.length || supports.length) {
    ...
    for (let i = 0; i < max; i++) {
      srRows.push({ resistance: resistances[i] ?? "", support: supports[i] ?? "" });
    }
...
export function buildSupportResistanceTableHtml(rows: SupportResistanceRow[]): string {
  if (!rows.length) return "";
  const body = rows
    .map(
      (r) =>
